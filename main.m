...
[audio_signals2 word_labels2] = load_audio_from_folder('newaudio');

display(sprintf('Loaded a total of %d audio signals for the following words:', length(audio_signals)))
display(unique(word_labels))
display(sprintf('Loaded a total of %d audio signals for the following words:', length(audio_signals2)))
display(unique(word_labels2))
...
% unpredict = vocabulary.additionaldata(word_labels2');
misses = 0;
...
%    celldisp(word_labels2);
%    word_labels2 = cell2str(word_labels2(1));
%     display(sprintf('Miss %d: .', word_labels2'))
end
misses =misses +10;
for i = 1:length(word_labels2)
display(sprintf(' %s: UnPredicted ',word_labels2{i:i} ))
% sprintf('%s ', word_labels2{:})
end
